...
def _tns(tenant_id: str) -> str:
    """Per-tenant namespace segment for per-lead keys.

    A lead's phone number is unique only *within* a tenant (see the
    (tenant_id, phone_number) constraint in the Phase 3 migration): two businesses
    may both talk to the same number. Prefixing every per-lead cache key with the
    tenant keeps one workspace's conversation/stage/profile/matches disjoint from
    another's — without it the same phone would collide across tenants and leak
    one customer's lead data into another's.
    """
    return f"reva:t:{tenant_id}"


def _conversation_key(tenant_id: str, phone_number: str) -> str:
    return f"{_tns(tenant_id)}:conversation:{phone_number}"


def _stage_key(tenant_id: str, phone_number: str) -> str:
    return f"{_tns(tenant_id)}:stage:{phone_number}"


def _lead_data_key(tenant_id: str, phone_number: str) -> str:
    return f"{_tns(tenant_id)}:lead:{phone_number}"
...
async def is_rate_limited(tenant_id: str, phone_number: str) -> bool:
    """
    Max 10 messages per minute per phone number, scoped to the owning tenant so a
    busy lead in one workspace can't throttle the same number in another.
    Protects against spam and runaway LLM costs.
    """
    try:
        client = await get_redis_client()
        key = f"{_tns(tenant_id)}:ratelimit:{phone_number}"
...
async def get_conversation_history(tenant_id: str, phone_number: str) -> list[dict[str, str]]:
    """
    Retrieve the full conversation history for a lead, scoped to its tenant.
    Returns an empty list if no history exists or Redis is unavailable.
    """
    try:
        client = await get_redis_client()
        raw = await client.get(_conversation_key(tenant_id, phone_number))
...
async def save_conversation_history(
    ...
    try:
        client = await get_redis_client()
        await client.setex(
            _conversation_key(tenant_id, phone_number),
            CONVERSATION_TTL,
...
async def clear_conversation(tenant_id: str, phone_number: str) -> None:
    """Delete conversation history — used when a lead restarts or qualifies out."""
    try:
        client = await get_redis_client()
        await client.delete(_conversation_key(tenant_id, phone_number))
...
async def get_lead_stage(tenant_id: str, phone_number: str) -> str:
    """
    ...
    try:
        client = await get_redis_client()
        stage = await client.get(_stage_key(tenant_id, phone_number))
...
async def set_lead_stage(tenant_id: str, phone_number: str, stage: str) -> None:
    ...
    try:
        client = await get_redis_client()
        await client.setex(_stage_key(tenant_id, phone_number), STAGE_TTL, stage)
...
async def get_lead_data(tenant_id: str, phone_number: str) -> dict[str, Any]:
    """Return the accumulated extracted lead data dict, or empty dict."""
    try:
        client = await get_redis_client()
        raw = await client.get(_lead_data_key(tenant_id, phone_number))
        return json.loads(raw) if raw else {}
    except RedisError as exc:
...
async def update_lead_data(tenant_id: str, phone_number: str, updates: dict[str, Any]) -> None:
    """
    ...
    """
    current = await get_lead_data(tenant_id, phone_number)
    merged = {**current, **{k: v for k, v in updates.items() if v is not None}}
    try:
        client = await get_redis_client()
        await client.setex(
            _lead_data_key(tenant_id, phone_number),
            CONVERSATION_TTL,
...
def _matches_key(tenant_id: str, phone_number: str) -> str:
    return f"{_tns(tenant_id)}:matches:{phone_number}"


async def cache_lead_matches(tenant_id: str, phone_number: str, matches: list[dict[str, Any]]) -> None:
    try:
        client = await get_redis_client()
        await client.setex(
            _matches_key(tenant_id, phone_number),
            CONVERSATION_TTL,
...
async def get_cached_lead_matches(tenant_id: str, phone_number: str) -> list[dict[str, Any]]:
    try:
        client = await get_redis_client()
        raw = await client.get(_matches_key(tenant_id, phone_number))
        return json.loads(raw) if raw else []
    except RedisError as exc:
